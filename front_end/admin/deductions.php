<?php
/**
 * Admin Deductions Management Page
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!AuthMiddleware::verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'create') {
            $sql = "INSERT INTO deductions (employee_id, type, amount, is_recurring, description, effective_from, effective_to) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            Database::query($sql, [
                $_POST['employee_id'],
                $_POST['type'],
            ...
            Database::query("DELETE FROM deductions WHERE id = ?", [$_POST['deduction_id']]);
            $message = 'Deduction deleted.';
        }
    }
}

// Get all deductions with employee info and role
$sql = "SELECT d.*, CONCAT(u.first_name, ' ', u.last_name) as employee_name, e.employee_code, u.role
        FROM deductions d
        JOIN employees e ON d.employee_id = e.id
        JOIN users u ON e.user_id = u.id
        ORDER BY d.created_at DESC";
$deductions = Database::query($sql)->fetchAll();

// Calculate category statistics
$categoryStats = [
    'all' => count($deductions),
    'employee' => 0,
    'manager' => 0,
    'pension' => 0,
    'insurance' => 0,
...
foreach ($deductions as $d) {
    if (isset($categoryStats[$d['type']])) {
        $categoryStats[$d['type']]++;
    }
    if ($d['role'] === 'manager') {
...
<head>
    ...
    <title>Deductions | PayrollPro Admin</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge { padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: 700; background: rgba(239,68,68,.2); color: #fca5a5; border: 1px solid rgba(239,68,68,.3); }
        ...
        .type-badge { padding: 6px 12px; border-radius: 12px; font-size: 12px; font-weight: 700; background: rgba(239,68,68,.15); color: #fca5a5; border: 1px solid rgba(239,68,68,.3); }
        .amount { font-weight: 700; color: #fca5a5; font-size: 15px; }
        .recurring { color: #fde047; font-weight: 600; }
        ...
        .form-group.checkbox label { margin-bottom: 0; text-transform: none; }
...
<body>
        ...
        <div class="layout">
            <aside class="sidenav">
                ...
                <nav>
                    ...
                    <a class="nav-link active" href="deductions.php"><span>➖</span> Deductions</a>
            ...
            <main class="main">
                <div class="page-header">
                    <div>
                        ...
                        <p class="lede">Manage deductions for all employees and managers</p>
                    </div>
                    <button class="btn-add" onclick="openModal()">+ Add Deduction</button>
                </div>
                ...
                <div class="filter-tabs">
                    ...
                    <div class="filter-tab" data-filter="pension" data-filter-type="type">
                        <span>🏦 Pension</span>
                        <span class="count"><?= $categoryStats['pension'] ?></span>
                    </div>
                    <div class="filter-tab" data-filter="insurance" data-filter-type="type">
                        <span>🛡️ Insurance</span>
                        <span class="count"><?= $categoryStats['insurance'] ?></span>
                    </div>
                    <div class="filter-tab" data-filter="loan" data-filter-type="type">
                        <span>💳 Loan</span>
                        <span class="count"><?= $categoryStats['loan'] ?></span>
                    </div>
                    <div class="filter-tab" data-filter="tax_adjustment" data-filter-type="type">
                        <span>📊 Tax Adjustment</span>
                        <span class="count"><?= $categoryStats['tax_adjustment'] ?></span>
                    </div>
                    <div class="filter-tab" data-filter="union_dues" data-filter-type="type">
                        <span>🤝 Union Dues</span>
                        <span class="count"><?= $categoryStats['union_dues'] ?></span>
                    </div>
                    <div class="filter-tab" data-filter="garnishment" data-filter-type="type">
                        <span>⚖️ Garnishment</span>
                        <span class="count"><?= $categoryStats['garnishment'] ?></span>
                    </div>
                ...
                <div class="table-container">
                    <div class="table-header">
                        ...
                        <span class="badge"><?= count($deductions) ?> records</span>
                    </div>
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody id="deductionsTable">
                            ...
                            <tr data-type="<?= htmlspecialchars($d['type']) ?>" data-role="<?= htmlspecialchars($d['role']) ?>">
                                <td>
                                    <?php if ($d['role'] === 'manager'): ?>
                                    ...
                                    <?php endif; ?>
                                    <?= htmlspecialchars($d['employee_name']) ?> <small style="color:#9fb4c7;">(<?= $d['employee_code'] ?>)</small>
                                </td>
                                <td><span class="type-badge"><?= ucfirst(str_replace('_', ' ', $d['type'])) ?></span></td>
                                <td class="amount">-$<?= number_format($d['amount'], 2) ?></td>
                                <td class="recurring"><?= $d['is_recurring'] ? '✓ Yes' : 'No' ?></td>
                                <td><?= date('M d, Y', strtotime($d['effective_from'])) ?></td>
                                <td><?= $d['effective_to'] ? date('M d, Y', strtotime($d['effective_to'])) : 'Ongoing' ?></td>
                                <td>
                    ...
                    <?php if (empty($deductions)): ?><div class="empty-state"><p>No deductions yet. Click "Add Deduction" to create one.</p></div><?php endif; ?>
                </div>
            </main>
        </div>
    </div>
    ...
    <div class="modal" id="deductionModal">
        <div class="modal-content">
            <div class="modal-header"><h2>Add Deduction</h2><button class="modal-close" onclick="closeModal()">&times;</button></div>
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="description" placeholder="e.g., Health insurance premium">
                </div>
                <div class="form-group">
    ...
    <script>
        function openModal() { document.getElementById('deductionModal').classList.add('active'); }
        function closeModal() { document.getElementById('deductionModal').classList.remove('active'); }
        document.getElementById('deductionModal').addEventListener('click', function(e) { if (e.target === this) closeModal(); });
        
        // Category filtering
        const filterTabs = document.querySelectorAll('.filter-tab');
        const tableRows = document.querySelectorAll('#deductionsTable tr');
